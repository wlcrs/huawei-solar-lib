...
device_id = 0
client = await AsyncHuaweiSolar.create("192.168.200.1", 6607, device_id)

# Reading a single register

result = await bridge.client.get(rn.NB_PV_STRINGS, device_id)
...
results = await self.client.get_multiple([rn.LINE_VOLTAGE_A_B, rn.LINE_VOLTAGE_B_C, rn.LINE_VOLTAGE_C_A], self.device_id)
